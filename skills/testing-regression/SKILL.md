...
description: Use when the task is to run or plan regression coverage after changes, releases, or fixes, especially when existing functionality must be revalidated rather than when writing new feature-specific tests.
license: MIT
...
- Do not use when the main task is root-cause debugging; prefer debugging-first workflows there.
- Do not use for Android-, browser-, or game-specific testing if a dedicated domain testing skill applies.
- Do not use when the task is about designing new test architecture rather than validating existing behavior.
