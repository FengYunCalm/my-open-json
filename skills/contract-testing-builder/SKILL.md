...
description: Use when the user asks for contract testing, Pact, API contract verification, or consumer-driven contracts, especially when provider-consumer compatibility and breaking-change prevention are the main goals.
---
...
Use when the user:
- asks for contract testing, Pact, or consumer-driven contracts
- needs provider-consumer compatibility checks for an API
- wants to prevent breaking changes with explicit contract verification

## Boundaries

- Do not use for generic integration testing when no provider-consumer contract workflow is required.
- Do not use for ordinary API unit tests unless the request is specifically about contract guarantees.
