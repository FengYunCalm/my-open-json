...
- Use this only when graphify is installed and the current project actually maintains `graphify-out/`.
- Treat this as a graphify workflow skill, not as a generic code search replacement.
...
"$PYTHON" -c "import graphify" 2>/dev/null || "$PYTHON" -m pip install graphify -q 2>/dev/null || "$PYTHON" -m pip install graphify -q --break-system-packages 2>&1 | tail -3
